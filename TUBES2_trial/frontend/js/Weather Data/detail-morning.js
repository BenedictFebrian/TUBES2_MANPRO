async function start() {
  ...
  try {
    ...
    if (!data.result) {
      ...
      document.querySelector('.content > .details.left > h1').innerHTML = `${data.result.Temp9am}°`;

      document.querySelector('.content > .details.left > .info-right').innerHTML = `
        <strong>
          <br>
					<br>
					${data.result.WindDir9am} ${data.result.WindSpeed9am} km/h<br>
					${data.result.WindGustDir} ${data.result.WindGustSpeed} km/h<br> 
					${data.result.Humidity9am}%<br>
					${data.result.Pressure9am} Pa<br>
					${data.result.Cloud9am * 8}%<br>
